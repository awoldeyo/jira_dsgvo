...
def getForwardChanges():
    ...
    fw_df = fw_df[(fw_df['date'] >= pd.datetime.today() - Day(7)) & (fw_df['Status'] == fw_df['to'])]
...
def getBackwardChanges():
    ...
    bw_df = bw_df[(bw_df['date'] >= pd.datetime.today() - Day(7)) & (bw_df['from']>=bw_df['to'])].sort_values(['key','date'])
...
writer = pd.ExcelWriter(f'Report_status_changes_7_days_{current_date}.xlsx')
